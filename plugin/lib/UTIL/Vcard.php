...
class vCard
{
     ...
     * Streams the vcard to the browser client.
     */
    function save($caminho, $prefixo)
    {
        if (!$this->card) {
            $this->build();
        }
        if (!$this->filename) {
            $this->filename = $prefixo . "_" . $this->data['display_name'];
        }
        $this->filename = str_replace(' ', '_', $this->filename);
        $myfile = fopen($caminho . "/" .  $this->filename . ".vcf", "w") or die("Unable to open file!");
		fwrite($myfile, $this->card);
		fclose($myfile);
        return TRUE;
    }
